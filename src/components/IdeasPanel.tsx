  ...
  return (
    <div className="panel fade-up">
          ...
          <div key={i} className="py-1.5 px-2 rounded text-xs" style={{ background: "var(--surface-2)" }}>
            <div className="flex items-center gap-1.5">
              <div
                ...
                style={{ background: sectionColors[idea.section] || "var(--accent)" }}
              />
              <span className="font-medium" style={{ fontSize: "11px" }}>
                {idea.name}
              </span>
            </div>
            <p className="ml-2.5 mt-0.5" style={{ color: "var(--text-dim)", fontSize: "10px" }}>
              {idea.description}
            </p>
          </div>
        ))}
      ...
      {ideas.length > 6 && (
        ...
        >
          {showAll ? "Collapse" : `+${ideas.length - 6} more`}
        </button>
      )}
    </div>
  );
